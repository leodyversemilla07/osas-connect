...
export function useProfileForm(initialData: ProfileFormData, isStudent: boolean) {
    ...
    const handleCourseChange = useCallback((courseValue: string) => {
        setData(prevData => {
            ...
            const requiresMajor = courseValue === "Bachelor of Secondary Education" ||
                courseValue === "Bachelor of Science in Entrepreneurship";
    ...
    const submitForm = useCallback((onStepError: (step: number) => void) => {
        ...
        const processedData = {
            ...
            onError: (errors: ValidationErrors) => {
                ...
                if (errors.total_annual_income || errors.total_monthly_expenses ||
                    errors.house_rental || errors.food_grocery) {
                ...
                } else if (errors.civil_status || errors.sex || errors.date_of_birth ||
                    errors.mobile_number || errors.religion || errors.street || 
                    errors.barangay || errors.city || errors.province || errors.zip_code ||
                    errors.place_of_birth || errors.residence_type) {
