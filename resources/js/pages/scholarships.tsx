...
import { ArrowRight, Calendar, Filter, GraduationCap, Search } from 'lucide-react';
...
function ScholarshipCard({ scholarship, isAuthenticated }: { scholarship: Scholarship; isAuthenticated: boolean }) {
    ...
    return (
        <Card className={`overflow-hidden rounded-lg border border-[#010002]/10 dark:border-[#f3f2f2]/10 ${colors.card}`}>
            <CardContent className="p-6">
                <div className="flex items-center gap-2">
                    <GraduationCap className={`h-5 w-5 ${colors.icon}`} />
                    <h3 className="font-semibold">{scholarship.name}</h3>
                </div>
                <p className="text-muted-foreground mt-2 text-sm">{scholarship.description}</p>
                <Badge className={`mt-3 ${colors.badge}`}>{scholarship.type}</Badge>
                ...
                <div className="mt-4">
                    <h4 className="mb-2 text-sm font-medium text-gray-900 dark:text-gray-100">Program Requirements</h4>
                    ...
                    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                        ...
                        <section className="mt-8 py-4">
                            <div className="mb-12 text-center">
                                <span className="mb-3 inline-block rounded-full bg-[#23b14d]/10 px-4 py-1 text-sm font-medium text-[#23b14d]">
                                    Browse Scholarships
                                </span>
                                <h2 className="text-3xl font-bold text-[#005a2d]">Available Scholarships</h2>
                                <p className="mx-auto mt-4 max-w-2xl text-lg text-[#010002]/70 dark:text-[#f3f2f2]/70">
                                    {isAuthenticated
