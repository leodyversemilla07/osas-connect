...
// Step titles for the progress indicator - moved outside component to prevent re-creation
const STEP_TITLES = [
...
export default function Register() {
    ...
    // Field groups to validate for each step - defined outside the validation function to prevent recreation
    const STEP_FIELDS = useMemo(() => ({
        ...
        ACADEMIC: ['student_id', 'course', 'year_level'],
        ACCOUNT: ['email', 'password', 'password_confirmation']
    } as const), []);

    // Validate current step before proceeding to next - memoized to prevent recreations
    const validateStep = useCallback((step: number): boolean => {
        try {
            switch (step) {
                case 1: {
                    // Validate Personal Information fields
                    if (STEP_FIELDS.PERSONAL.some(field => !data[field as keyof typeof data])) {
                        return false;
                    }
                ...
                case 2: {
                    ...
                    return STEP_FIELDS.ACADEMIC.every(field => !!data[field as keyof typeof data]);
                }

                case 3: {
                    // Validate Account Setup fields
                    if (STEP_FIELDS.ACCOUNT.some(field => !data[field as keyof typeof data])) {
                        return false;
                    }
    ...
    }, [currentStep]);    // Memoized event handlers to prevent infinite re-renders
    // Remove setData from dependency arrays as it's stable from Inertia.js
    ...
    const handleCourseChange = useCallback((newCourseValue: string) => {
        setData(prevData => {
            const newMajor = (newCourseValue !== "Bachelor of Secondary Education" && newCourseValue !== "Bachelor of Elementary Education") ? 'None' : prevData.major;
            return {
                ...prevData,
    ...
    }, [setData]);    const handleTermsAgreementChange = useCallback((checked: boolean | 'indeterminate') => {
        setData('terms_agreement', checked === true);
    }, [setData]);

    // Additional memoized handlers for remaining inputs
    const handleLastNameChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
        setData('last_name', e.target.value);
    }, [setData]);    const handleFirstNameChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    }, [setData]);    const handleScholarshipsChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
        setData('scholarships', e.target.value);
    }, [setData]);

    const handleTelephoneNumberChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
        setData('telephone_number', e.target.value);
    }, [setData]);    const handleDisabilityTypeChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
        setData('disability_type', e.target.value);
    }, [setData]);

    // Memoized handlers for radio groups and selects - single setData calls only
    // Remove setData from dependency arrays as it's stable from Inertia.js
    const handleResidenceTypeChange = useCallback((newResidenceTypeValue: string) => {
        setData(prevData => ({
            ...prevData,
            residence_type: newResidenceTypeValue,
            guardian_name: newResidenceTypeValue === 'With Guardian' ? '' : 'Not Applicable',
        }));
    }, [setData]); // setData is stable

    const handlePwdChange = useCallback((newIsPwdValue: string) => {
        setData(prevData => {
            const newDisabilityType = newIsPwdValue === 'No' ? '' : prevData.disability_type;
            return {
                ...prevData,
                is_pwd: newIsPwdValue,
                disability_type: newDisabilityType,
            };
        });
    }, [setData]); // data.disability_type is no longer read directly from component scope

    const handleSexChange = useCallback((value: string) => {
        setData('sex', value);
    }, [setData]);

    const handleCivilStatusChange = useCallback((value: string) => {
        setData('civil_status', value);
    }, [setData]);

    const handleReligionChange = useCallback((value: string) => {
        setData('religion', value);
    }, [setData]);    const handleYearLevelChange = useCallback((value: string) => {
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="grid gap-2">
                                ...
                                >
                                    <div className="flex items-center space-x-2">
                                        ...
                                        <Label htmlFor="parents-house" className="cursor-pointer">Parent's House</Label>
                                    </div>
                                    <div className="flex items-center space-x-2">
                                        <RadioGroupItem value="Boarding House" id="boarding-house" />
                                        <Label htmlFor="boarding-house" className="cursor-pointer">Boarding House</Label>
                                    </div>
                                    <div className="flex items-center space-x-2">
                                        <RadioGroupItem value="With Guardian" id="with-guardian" />
                                        <Label htmlFor="with-guardian" className="cursor-pointer">With Guardian</Label>
                                    </div>
                                </RadioGroup>
                            ...
                            <div className="grid gap-2">
                                ...
                                <Input
                                    ...
                                    onChange={handleScholarshipsChange}
                                    disabled={processing}
                            ...
                            {data.is_pwd === 'Yes' && (
                                <div className="grid gap-2">
                                    ...
                                    />
                                    <p className="text-xs text-muted-foreground">
                                        Specify your type of disability to help us better accommodate your needs
                                    </p>
                            ...
                            <div className="rounded-lg border p-4 space-y-4">
                                <div>
                                    ...
                                    <div className="grid grid-cols-2 gap-2 mt-2 text-sm">
                                        ...
                                        <p>{data.date_of_birth ? new Date(data.date_of_birth).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' }) : 'Not provided'}</p>
                                        ...
                                        <p>{data.telephone_number || 'Not provided'}</p>

                                        <p className="text-muted-foreground">Religion:</p>
                                        <p>{data.religion}</p>

                                        <p className="text-muted-foreground">PWD:</p>
                                        <p>{data.is_pwd} {data.is_pwd === 'Yes' && data.disability_type && `(${data.disability_type})`}</p>
