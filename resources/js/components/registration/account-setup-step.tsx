...
const AccountSetupStep = memo<AccountSetupStepProps>(({ data, errors, onFieldChange }) => {
    const isEmailValid = data.email && data.email.toLowerCase().endsWith(VALIDATION.EMAIL.DOMAIN);
    const doPasswordsMatch = data.password && data.password_confirmation && data.password === data.password_confirmation;
    ...
    return (
        <div className="space-y-6">
            <div>
                ...
                />
                <p className="text-muted-foreground mt-1 text-sm">Must be a valid university email ending with {VALIDATION.EMAIL.DOMAIN}</p>
                <InputError id="email-error" message={errors.email} className="mt-1" />
                {data.email && !isEmailValid && !errors.email && (
                    <p className="mt-1 text-sm text-amber-600">Email must end with {VALIDATION.EMAIL.DOMAIN}</p>
                )}
            </div>
            ...
            <div>
                ...
                {data.password && data.password_confirmation && !doPasswordsMatch && !errors.password_confirmation && (
                    <p className="mt-1 text-sm text-amber-600">Passwords do not match</p>
                )}
                {data.password && data.password_confirmation && doPasswordsMatch && <p className="mt-1 text-sm text-green-600">Passwords match ✓</p>}
            </div>

            <div className="bg-muted rounded-lg border p-4">
                <h4 className="text-foreground mb-2 font-medium">Account Security Tips</h4>
