...
export const createColumns = (onViewDetails?: (scholarship: Scholarship) => void): ColumnDef<Scholarship>[] => [
    {
        ...
        cell: ({ row }) => (
            <div className="max-w-[300px]">
                ...
                <div className="line-clamp-2 text-sm text-gray-500 dark:text-gray-400">{row.original.description}</div>
            </div>
        ),
    },
    {
        accessorKey: 'type',
        header: 'Type',
        cell: ({ row }) => <Badge className={getTypeColor(row.getValue('type'))}>{row.getValue('type')}</Badge>,
    },
    {
        accessorKey: 'amount',
        header: 'Amount',
        cell: ({ row }) => (
            <div className="font-medium">
                {row.getValue('amount')}
                <div className="text-xs text-gray-500 dark:text-gray-400">{row.original.stipendSchedule}</div>
            </div>
        ),
    },
    {
    ...
    },
    {
        accessorKey: 'status',
        header: 'Status',
        cell: ({ row }) => <Badge className={getStatusColor(row.getValue('status'))}>{row.getValue('status') === 'open' ? 'Open' : 'Closed'}</Badge>,
    },
    {
        id: 'actions',
        enableHiding: false,
