    ...
    return (
        <Card className="w-full">
            <CardHeader>
                ...
                <CardDescription>Please upload a recent 1x1 ID photo with white background (max {maxSizeInMB}MB)</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
                ...
                <div
                    className={cn(
                        'relative mx-auto aspect-square w-full max-w-[300px] overflow-hidden rounded-lg border-2 transition-all duration-200',
                        photoPreview
                            ? 'border-border border-solid'
                            : 'border-muted-foreground/25 hover:border-muted-foreground/50 cursor-pointer border-dashed',
                ...
                >
                    {photoPreview ? (
                        <>
                            <img src={photoPreview} alt="1x1 ID Preview" className="h-full w-full object-cover" loading="lazy" />
                            <button
                                onClick={(e) => {
                                    e.stopPropagation();
                                    clearPreview();
                                ...
                                disabled={isLoading}
                                className="bg-background/80 hover:bg-background absolute top-2 right-2 rounded-full p-1 backdrop-blur-sm transition-colors disabled:opacity-50"
                        ...
                        <div className="flex h-full flex-col items-center justify-center p-4">
                            {isLoading ? (
                                ...
                                <>
                                    ...
                                    <p className="text-muted-foreground mb-2 text-center text-sm">Tap to upload your 1x1 photo</p>
                                    <p className="text-muted-foreground text-center text-xs">Square format, white background preferred</p>
                                </>
                            )}
                        </div>
                    )}
                ...
                {!photoPreview && !isLoading && (
                    <Button type="button" variant="outline" onClick={handleUploadClick} className="w-full" aria-label="Upload photo">
                ...
                <div id="photo-requirements" className="text-muted-foreground space-y-1 text-xs">
                    <p>Requirements:</p>
                    ...
                    <p className="mt-2">Your photo will be used for identification purposes in your student profile</p>
                </div>
            </CardContent>
        </Card>
    );
